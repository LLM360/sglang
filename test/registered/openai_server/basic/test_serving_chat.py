...
class ServingChatTestCase(unittest.TestCase):
    ...
    def test_k2v3_tracking_events_are_omitted_without_meta_info(self):
        self.chat.tool_call_parser = "k2_v3_tracking"
        req = ChatCompletionRequest(
            model="x",
            messages=[{"role": "user", "content": "Set flag"}],
        ...
        self.assertNotIn("tool_parser_fallback_events", ret[0]["meta_info"])

    def test_k2v3_reasoning_tracking_events_are_omitted_without_meta_info(self):
        self.chat.reasoning_parser = "k2_v3_tracking"
        self.template_manager.force_reasoning = True
        req = ChatCompletionRequest(
            model="x",
        ...
        self.assertIsNone(response.choices[0].meta_info)
        self.assertNotIn("reasoning_parser_fallback_events", ret[0]["meta_info"])

    def test_k2v3_fallback_event_attachment_deep_copies_events(self):
        parser = Mock()
        parser.detector = Mock()
        parser.detector.fallback_events = [
            {"type": "event", "phase": "phase", "details": {"items": []}}
        ]
        meta_info = {}

        self.chat._attach_tool_parser_fallback_events(meta_info, parser)
        parser.detector.fallback_events[0]["details"]["items"].append("mutated")

        self.assertEqual(
            meta_info["tool_parser_fallback_events"][0]["details"]["items"], []
        )
        ...
        self.chat._attach_reasoning_parser_fallback_events(meta_info, parser)
        parser.detector.fallback_events[0]["details"]["items"].append("mutated")

        self.assertEqual(
            meta_info["reasoning_parser_fallback_events"][0]["details"]["items"], []
        )
