...
Ported from LLM360/vllm's MultiFormatToolParser, including PR #12.
...
Streaming:
  ...
  - Embedded dialects buffer from the first possible tool marker and repeatedly
    use the non-streaming parser. K2-V3 further requires a complete plural
    <ifm|tool_calls> wrapper before emitting calls. On normal completion,
    unresolved markup is released as ordinary content.
"""
...
class MultiFormatDetector(BaseFormatDetector):
    ...
    def parse_streaming_increment(
        ...
        return self._streaming_increment_fallback(new_text, tools)

    def finalize_streaming(self, tools: List[Tool]) -> StreamingParseResult:
        if self._delegate is not None:
            return self._delegate.finalize_streaming(tools)
    ...
    def _partial_tool_call_marker_start(self, text: str) -> Optional[int]:
    ...
    def _try_emit_streaming_tool_calls(
        self, tools: List[Tool], content: Optional[str] = None
    ) -> StreamingParseResult:
        if not self._streaming_input_complete():
            return StreamingParseResult(normal_text=content, calls=[])

        parsed = self.detect_and_parse(self._streaming_content_buffer, tools)
        if not parsed.calls:
            return StreamingParseResult(normal_text=content, calls=[])

        new_calls = parsed.calls[self._streaming_tool_calls_emitted :]
        if not new_calls:
            return StreamingParseResult(normal_text=content, calls=[])

        for call in new_calls:
            call.tool_index = self._streaming_tool_calls_emitted
        ...
        return StreamingParseResult(normal_text=content, calls=new_calls)

    def _streaming_input_complete(self) -> bool:
        return True

    def _streaming_increment_fallback(
        self, new_text: str, tools: List[Tool]
    ) -> StreamingParseResult:
        self._streaming_content_buffer += new_text

        if self._streaming_tool_call_started:
            return self._try_emit_streaming_tool_calls(tools)

        buffer = self._streaming_content_buffer
        markers = self._tool_call_markers()
        marker_positions = [
            position for marker in markers if (position := buffer.find(marker)) != -1
        ]
        if marker_positions:
            first_marker_position = min(marker_positions)
            self._streaming_tool_call_started = True
            content = buffer[:first_marker_position]
            self._streaming_content_buffer = buffer[first_marker_position:]
            return self._try_emit_streaming_tool_calls(tools, content or None)
        ...
        if partial_start is not None:
            ...
            return StreamingParseResult(normal_text=content or None, calls=[])

        self._streaming_content_buffer = ""
        return StreamingParseResult(normal_text=buffer or None, calls=[])
...
class K2V3Detector(MultiFormatDetector):
    ...
    def has_tool_call(self, text: str) -> bool:
        ...
        contains_ifm_marker = (
            self._IFM_TOOL_CALLS_START_TOKEN in text
            or self._IFM_TOOL_CALL_START_TOKEN in text
        )
        if uses_ifm_format or (self.tool_format == "glm" and contains_ifm_marker):
            wrapper_start = text.find(self._IFM_TOOL_CALLS_START_TOKEN)
            if wrapper_start == -1:
                return False
            return (
                text.find(
        ...
        return super().has_tool_call(text)
    ...
    def detect_and_parse(self, text: str, tools: List[Tool]) -> StreamingParseResult:
        ...
        if uses_ifm_format or (self.tool_format == "glm" and contains_ifm_marker):
            ...
            extracted = super().detect_and_parse(text[wrapper_start:wrapper_end], tools)
            if not extracted.calls:
                return StreamingParseResult(normal_text=text, calls=[])
            prefix = self._strip_ifm_reasoning_prefix(text[:wrapper_start])
            extracted.normal_text = prefix
            return extracted

        extracted = super().detect_and_parse(text, tools)
        if extracted.calls and extracted.normal_text is None:
