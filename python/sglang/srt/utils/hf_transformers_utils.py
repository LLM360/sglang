...
try:
    ...
    # Older huggingface_hub releases do not expose strict dataclass support.
    class StrictDataclassDefinitionError(Exception):
        pass
...
class _K2AuroraConfig(PretrainedConfig):
    """Native-only config shell for canonical K2Aurora checkpoint fields."""
    ...
    **kwargs,
    ...
    if (
        ...
        try:
            ...
            # Canonical K2Aurora artifacts generated by the Transformers-5
            # modular converter decorate their PretrainedConfig subclass with
            # huggingface_hub.dataclasses.strict without first making it a
            # dataclass.  Newer hub releases reject that remote module before
            # SGLang can select its native implementation.  Native inference
            # only needs the persisted fields, so load the raw dict while
            # keeping this fallback exact to the affected artifact contract.
            config_dict, _ = PretrainedConfig.get_config_dict(
                model,
                revision=revision,
                **kwargs,
            ...
            if not (
                config_dict.get("model_type") == "k2_aurora"
                and config_dict.get("architectures") == ["K2AuroraForCausalLM"]
            ):
                raise
            config = _K2AuroraConfig.from_dict(config_dict)
            config._name_or_path = model
